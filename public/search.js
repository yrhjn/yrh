...
async function continueToNextStep() {
    ...
    // workDate から searchDate までの範囲に一致するデータを抽出
    const matchingData = data.filter(entry => entry.date >= workDate && entry.date <= searchDate);
  ...
  function displayDataInTable(data) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (i < data.length) {
        const entry = data[i];
        // 列の順番を変更してデータを表示
        const columnsOrder = ['_id','date', 'dropOffDate', 'dispatch', 'carNumber', 'driver', 'hirenumber', 'hireCompany', 'customernumber', 'customer', 'departure', 'destination','date4','date1','date3','date2','date5', /* その他のカラム */];
  ...
  async function saveAndLoadData() {
    // 保存処理
    await saveInputForm();
  ...
  // 車番号が変更されたときに呼び出される関数
  async function onCarNumberChange() {
    ...
    // サーバーに対してMongoDBからデータを要求
    const response = await fetch('/loadData'); // データの取得先によって変更
    if (!response.ok) {
      throw new Error('データの読み込みに失敗しました。');
    }
    ...
    // 入力されたcarNumberに一致するデータを検索
    const matchingData = data.find(entry => entry.savecarNumber === carNumber);
  
    if (matchingData) {
      // ドライバーを反映
      driverInput.value = matchingData.savedriver;
    } else {
      // データが見つからなかった場合の処理
      driverInput.value = "";
    }
  ...
  document.addEventListener("DOMContentLoaded", function() {
    // 車番号の入力欄に変更イベントを追加
    const carNumberInput = document.getElementById('car-number');
    carNumberInput.addEventListener('input', onCarNumberChange);
  });
  ...
  function reflectDataInInputForm(data) {
    // カラムの順番
    const columnsOrder = ['_id','new-date', 'drop-off-date', 'dispatch', 'car-number', 'driver', 'hire-number', 'hire-company', 'customernumber', 'customer', 'departure', 'destination','date4','date1','date3','date2','date5', /* その他のカラム */];
...
// 新しく追加：選択された行をマークする関数
function markSelectedRow(row) {
  // 以前の選択を解除
  var selectedRows = dataTable.querySelectorAll(".selected");
...
async function deleteData() {
  ...
  if (selectedData) {
      try {
          const _id = selectedData[0];  // _idのインデックスに変更する必要があります

          // サーバーに対してデータ削除のリクエストを送信
          ...
          if (response.ok) {
            ...
            console.error('データの削除中にエラーが発生しました。', response.status, response.statusText);
          }
      ...
      console.warn('削除対象のデータが選択されていません。');
  }
}
...
function setDropOffDate() {
  // new-dateの値を取得
  var newDateValue = document.getElementById('new-date').value;

  // drop-off-dateに同じ値を設定
  document.getElementById('drop-off-date').value = newDateValue;
  document.getElementById('loading-date').value = newDateValue;
}
