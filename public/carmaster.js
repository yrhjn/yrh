...
async function saveInputForm() {
    const savecarNumber = document.getElementById('savecar-number').value; // 車番
    const savedriver = document.getElementById('savedriver').value; // ドライバー名
    ...
    const response = await fetch('/saveData', {
      ...
      body: JSON.stringify({
        savecarNumber: savecarNumber, // 車番
        savedriver: savedriver, // ドライバー名
      }),
    });
